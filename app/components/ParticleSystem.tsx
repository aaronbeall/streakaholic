...
const ParticleComponent = React.memo(({ particle }: { particle: Particle }) => {
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    const swirlOffset = swirlEnvelope * particle.swirlAmplitude *
    ...
    return {
      transform: [
        { translateX: particle.originX + particle.driftX * t + particle.perpX * swirlOffset },
        { translateY: particle.originY + particle.driftY * t + particle.perpY * swirlOffset },
...
const defaultParticles: ParticleConfig = {
  color: '#FF6B6B',
  // Skewed red rather than an even hue spread -- `#FF6B6B` (repeated for weight) matches the
  // streak badge's own "fire" red elsewhere in `TaskCard`, with a couple of darker/oranger reds
  // as accents rather than true yellow/gold, so most particles read as red with only occasional
  // variation instead of an even red-to-yellow gradient.
  colors: ['#FF6B6B', '#FF6B6B', '#FF3B1F', '#FF3B1F', '#FF6B00'],
  colorVariance: 12,
  size: 13,
  sizeVariance: 9,
  distance: 18,
...
export const ParticleSystem = React.memo(({
  ...
  const createParticle = useCallback((): Particle => {
    ...
    const baseColor = config.colors && config.colors.length > 0
    ...
    return {
      ...
      swirlAmplitude: getRandomValue(config.swirlAmplitude, config.swirlAmplitudeVariance),
      swirlFrequency: getRandomValue(config.swirlFrequency, config.swirlFrequencyVariance),
  ...
  const [particleState] = useState<Particle[]>(() =>
    Array.from({ length: count }, createParticle).sort((a, b) => a.spawnDelay - b.spawnDelay)
  );
  ...
  useEffect(() => {
    const timeouts = particleState.map((particle, index) =>
      setTimeout(() => setVisibleCount(v => Math.max(v, index + 1)), particle.spawnDelay)
    );
